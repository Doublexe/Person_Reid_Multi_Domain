...
def func_parse(root, new_root, func, save_func, filt='**/*'):
    """
    root/hier/file1 -> new_root/hier/file1
    """
    filenodes = parse_hierachy(root, filt)
    for filenode in filenodes:
        new_dir = filenode.echo(new_root)
        result = func(filenode.path)
        save_func(os.path.join(new_dir, filenode.file_name), result)
