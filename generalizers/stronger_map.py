...
class Generalizer_D(BasicModule):
    def __init__(self, num_domains):
        ...
        def discriminator_block(in_filters, out_filters, bn=True):
            block = [nn.Conv2d(in_filters, out_filters, 2, 1, 0), nn.LeakyReLU(0.2, inplace=True)]
            if bn:
                block.append(nn.BatchNorm2d(out_filters, 0.8))
            return block

        self.conv_blocks = nn.Sequential(
            *discriminator_block(in_channels, 512),
            *discriminator_block(512, 512),
            *discriminator_block(512, 512),
            *discriminator_block(512, 128),
            *discriminator_block(128, 32),
        )
        ...
        self.valid = nn.Sequential(
            ...
            # nn.Linear(512, 64),
            # nn.LeakyReLU(),
            nn.Linear(1056, num_domains)  # 12-2 * 4-2 = 10 * 2
        )
