...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, IN=False, leaky=False, collect=False):
    ...
    def forward(self, x):
        ...
        if not self.collect:
            ...
            c.append(self.avg(out).view(out.shape[0],-1))
...
class Generalizer_G(BasicModule):
    ...
    def p2(self, x):
        ...
        x = self.layer2(x) #[256, 64, 32]
        x = self.layer3(x) #[512, 64, 32]
        x = self.layer4(x) #[1024, 64, 32]
        return x
    ...
    def forward(self, x):
        ...
        feat_maps = self.p1(x)
        fs.append(feat_maps)
        feat_maps = self.layer1(feat_maps)
        fs.append(feat_maps)
        feat_maps = self.layer2(feat_maps)
        fs.append(feat_maps)
        feat_maps = self.layer3(feat_maps)
        fs.append(feat_maps)
        feat_maps = self.layer4(feat_maps)
        # fs.append(feat_maps)
        f = feat_maps
...
#             # nn.Linear(512, 64),
#             # nn.LeakyReLU(),
#             nn.Linear(260, num_domains)  # 12-2 * 4-2 = 10 * 2
#         )
#
#     def forward(self, x):
#         x = self.conv_blocks(x).view(x.shape[0], -1)
...
class Generalizer_D(BasicModule):
    ...
    def __init__(self, num_domains,
                 ...
                 **kwargs):
        self.inplanes = 64
        super(Generalizer_D, self).__init__()
        self.feature_dim = 512 * block.expansion
        ...
        # self.relu = nn.ReLU(inplace=True)
        # self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.layer1 = self._make_layer(block, 64, layers[0],IN=True, first_layer = True)
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2,IN=True)
        self.layer3 = self._make_layer(block, 256, layers[2], stride=2,IN=True)
        ...
        self.fn = nn.BatchNorm1d(self.feature_dim)
        self.fn.bias.requires_grad_(False)  # no shift
        self.classifier = nn.Linear(self.feature_dim, num_domains, bias=False)
        self.global_avgpool = wavg
    ...
    def _make_layer(self, block, planes, blocks, stride=1,IN=False, first_layer=False):
        degree = 1 if first_layer else 2
        downsample = None
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                nn.Conv2d(self.inplanes*degree, planes * block.expansion,
                          kernel_size=1, stride=stride, bias=False),
                nn.BatchNorm2d(planes * block.expansion),
            )

        layers = []
        layers.append(block(self.inplanes*degree, planes, stride, downsample, leaky=True))
        self.inplanes = planes * block.expansion
        for i in range(1, blocks-1):
            layers.append(block(self.inplanes, planes, leaky=True))
        layers.append(block(self.inplanes,planes,IN=IN, leaky=True))
    ...
    def forward(self, fs):
        # fs.append(feat_maps)
        feat_maps = self.layer1(fs[0])
        # fs.append(feat_maps)
        feat_maps = self.layer2(torch.cat([feat_maps, fs[1]], dim=1))
        # fs.append(feat_maps)
        feat_maps = self.layer3(torch.cat([feat_maps, fs[2]], dim=1))
        # fs.append(feat_maps)
        feat_maps = self.layer4(torch.cat([feat_maps, fs[3]], dim=1))
        # fs.append(feat_maps)
        f = feat_maps
        # fs = torch.cat(fs, dim=1)
        v = self.global_avgpool(f)
